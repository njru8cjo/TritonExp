...
def compile_module(launcher_src, kernel_placeholder_name):
    ...
    def launch(
        ...
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "main.cxx")).write_text(src)
        #Path("/home/ywshao/kernel.s").write_bytes(asm_src)
        ...
        if cache_path is None:
          with tempfile.TemporaryDirectory() as tmpdir:
              ...
              asm_src_path = os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "kernel.s")
              launcher_src_path = os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "main.cxx")

              #so_path = os.path.join(tmpdir, "kernel.so")
              so_path = os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "kernel.so")
              Path(asm_src_path).write_bytes(asm_src)
              #Path(launcher_src_path).write_text(src)
              # Compile it together.
              subprocess.check_call([
                "/opt/riscv/bin/riscv64-unknown-linux-gnu-g++", "-std=c++17", launcher_src_path, asm_src_path,
                f"-I{py_include_dir}", f"-I{include_dir}",
                # Cross compile for BPI F3
                #"--target=riscv64-linux-gnu",
                "-march=rv64gcv_zvl256b",
                #"-mabi=lp64d",
                #"--sysroot=/opt/riscv/sysroot"
                "-nostdinc",
                "-I/opt/riscv/riscv64-unknown-linux-gnu/include/c++/13.2.0",
                "-I/opt/riscv/riscv64-unknown-linux-gnu/include/c++/13.2.0/riscv64-unknown-linux-gnu",
                "-I/opt/riscv/sysroot/usr/include",
                "-I/opt/riscv/lib/gcc/riscv64-unknown-linux-gnu/13.2.0/include",
                #"-I/usr/riscv64-linux-gnu/include",
                #No x86 #"-I/usr/include/x86_64-linux-gnu/c++/13",
                "-I/usr/include", #Python.h and riscv64-linux-gnu/python3.12/pyconfig.h
                #"-L/usr/riscv64-linux-gnu/lib",
                "-L" + os.getenv("RISCV_GCC_LIB", ""),
                "-L" + os.getenv("RISCV_SYSROOT_LIB", ""), #libomp.so
                #
                #"-fno-rtti",
                #"-print-search-dirs",
                "-lstdc++",
                "-shared", "-lomp", # Use libomp instead of libgomp to match clang
                "-O2",
                "-fPIC", "-fopenmp", "-o", so_path
              ])
              print("\nCongratulations! You have successfully got the shared library \"kernel.so\"! I guess.\n")
        ...
        print("It cannot load kernel.so for some reason. Maybe arch or permission? RISC-V binary cannot run on x86 though.")
        print("A \"temporary\" solution would be use a path except \"temp\" (cache.get_file(filename)).")
        return
...
class CPULauncher(object):
    ...
    def __init__(self, src, metadata):
        ...
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "kernel.cpp")).write_text(launcher_src)
