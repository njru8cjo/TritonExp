...
def _ttir_to_ttsharedir(mod):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "ttir.mlir")).write_text(ttir_code) #
...
def _optimize_ttsharedir(ttsharedir: str):
    print("_optimize_ttsharedir")
    # We don't apply any optimizations now, but we can add passes if needed.
    Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "ttsharedir.mlir")).write_text(ttsharedir) #
    return ttsharedir


def _ttsharedir_to_scfir(ttsharedir: str):
    print("_ttsharedir_to_scfir")
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "scf.mlir")).write_text(Path(scfmlir_path).read_text()) #
        return Path(scfmlir_path).read_text()


def _optimize_scfir(scfir: str):
    print("_optimize_scfir")
    # Serialize inner scf.parallel
    with tempfile.TemporaryDirectory() as tmpdir:
        scfmlir_path = os.path.join(tmpdir, "scf.mlir")
        scf2mlir_path = os.path.join(tmpdir, "scf2.mlir")
        Path(scfmlir_path).write_text(scfir)
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "scf.mlir")).write_text(scfir) #
        subprocess.check_call([os.environ.get("LOOP_SERIALIZATION_DIR"), scfmlir_path,
                "--inner-loop-serialization",
                "-o",
                scf2mlir_path])
    
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "scf_after.mlir")).write_text(Path(scf2mlir_path).read_text()) #
...
def _scfir_to_llir(scfir: str):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "ll.mlir")).write_text(Path(llmlir_path).read_text()) #
...
def _optimize_llir(llir: str):
    print("_optimize_llir")
    # We don't apply any optimizations now, but we can add passes if needed.
    Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "ll.ll")).write_text(llir) #
    return llir
...
def _llir_to_bin(llir: str, metadata):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        #subprocess.check_call([llc_path, "-mtriple=riscv64-linux-gnu", "-march=rv64gcv_zvl256b", src_path, "-o", dst_path])
        print("\nllc\n")
        clang_path = _get_llvm_bin_path("clang")
        #subprocess.check_call([clang_path, src_path, "-o", dst_path, "-S", "-fPIC", "-fopenmp"])
        subprocess.check_call([os.getenv("RISCV_CLANG", ""), src_path, "-o", dst_path,
                               "--target=riscv64-linux-gnu", "-march=rv64gcv_zvl256b",
                               #"-loop-vectorize",
                               "-fno-addrsig",
        ...
        Path(os.path.join(os.getenv("TRITON_OUTPUT_DIR", ""), "kernel.o")).write_text(Path(dst_path).read_text()) #
        return Path(dst_path).read_text()
